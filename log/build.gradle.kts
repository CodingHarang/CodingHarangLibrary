...
android {
    ...
    buildFeatures {
        buildConfig = true // BuildConfig 생성 활성화 (Kotlin/Java 코드에서 BuildConfig 참조 가능)
    }
    ...
    publishing {
        publications {
            create<MavenPublication>("release") {
                ...
                artifactId = "log" // 모듈 artifactId
                version = project.version.toString() // 상단 version과 동일
                pom {
                    name.set("Log") // POM name
                    description.set("CodingHarang Log Library") // POM description
